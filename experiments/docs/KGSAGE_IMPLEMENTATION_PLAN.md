...
- **D positive class = (anchor, related-real-triple)** — a second real triple sharing h or (h,r) with the
  anchor (NOT (anchor, anchor): degenerate identity check; NOT the sampler target as now, L133-141).
  One-sided label smoothing 0.9; optional spectral norm on D (pick ONE stabilizer set up front).
...
- Per-epoch drift diagnostics (these become paper curves): pre-filter would-have-collided rate,
  fraction of candidates KGE-ranked ≤10 / above the true value, raw-argmax copy rate, D accuracy per class.
- Checkpoint: store type pools + arm/loss config; save **best-by-validation**, not last epoch.

**Accept:** all four arms train on fixture + dummy_kg without divergence; drift curves flat; D accuracy
neither 0.5 (artifact) nor 1.0 (collapse); fixture A5–A7 still pass through the full GAN.
...
`cli/inspect_corruptions.py`: **invert the diagnostic** — primary column becomes true-value similarity
(high = good) + band percentile + `neighbour_supported`; make it call the shared inference decode path so
reports reflect deployment. Pool-size distribution report per dataset; small pools augmented via KGE
...
`--test_gan_path` checkpoints must match `--dataset` (add a dataset-name + triple-set-hash guard in the
...
targets + fixture`, `M2: real-fact discriminator + arms`, `M3: pool-masked inference + null filtering`,

# Option A — LP-in-the-loop GAN (PoC stage 2, builds on OPTION_B_PLAN.md)

Feasibility workup 2026-07-03 (3-agent: equilibrium analysis, prior-art, code/compute). Verdict:
**feasible; primary design = A-ii (frozen ComplEx + trainable contextual residual discriminator);
...
spread — bounds the residual so total score can never leave the s_f band). D(x) = s_z(x) + f_θ(x).
...
- **Step 2 — adversarial loop (1:1 steps):**
  - D step (θ only): positives = train triples; fakes = 50% G hard samples + 25% band-sampler
...
- **5 per-epoch diagnostics:** truth-drift (fence-hit %, LP-top-1 rate; alarm >25%); residual health
  ...
  diversity (distinct fillers/relation, entropy, z-sensitivity); game balance (D-acc on G-slice
  0.6–0.8, alarm >0.95 for 3 epochs).
...
- **New/changed code:** ~+880 / ~380 modified / −185 deleted. New: `gan/complex_d.py` (~130, trainable
  ...
  (~90). Reworked: `train_one_epoch` (+170/~120/−45). Reused unchanged: KGSAGEGenerator, encoder.py,
...
anchor (RLHF-style, Ziegler et al. 2019 pattern), a relativistic frozen-score+residual discriminator
...
(the backbone of) the adversarial discriminator and as a frozen truth-fence, with straight-through

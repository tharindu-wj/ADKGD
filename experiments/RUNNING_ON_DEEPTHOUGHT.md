...
> **Steps 1–2 happen in the KGSAGE repository, not this one.** KGSAGE is a
> separate pip-installable package; this repo only consumes its checkpoints.
> Its own `README.md` and `slurm/README.md` are authoritative for them.

1. **Train the generator — Phases 1–2**, in the KGSAGE checkout (GPU
   `train.slurm`, or CPU `train_cpu.slurm` reusing a cached context table E'):
   `cd $KGSAGE_HOME && DATASET=fb15k237 SEED=0 sbatch slurm/train.slurm`. Phase
...
2. **Select the snapshot** (the anchor-knockout criterion; lowest mean knockout
   ...
   `python -m kgsage.cli.knockout_eval --ckpt <each .epNN.pt> --data
   data/FB15K-237` (WN18RR: pass `--relations _hypernym
   _derivationally_related_form _member_meronym _has_part`). Copy the winner
   into this repo at `artifacts/kgsage/` — see that folder's README. Requires
   `pip install -e $KGSAGE_HOME` in the same conda env used here.
...
> launcher in THIS repo is `experiments/slurm/exp_cell.slurm`, and generator
> training moved out entirely to the KGSAGE repo's `slurm/train{,_cpu}.slurm`.
> Read those filenames as

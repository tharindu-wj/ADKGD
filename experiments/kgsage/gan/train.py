"""Train the KGSAGE GAN (KGSAGEGenerator + KGSAGEDiscriminator) and save a checkpoint.
...
So `kgsage.inference` only needs the checkpoint, no separate data files.
"""
import argparse
import os
import random
...
from kgsage.gan.models import KGSAGEGenerator, KGSAGEDiscriminator, gumbel_softmax, soft_embedding
...
def main():
    ...
    G = KGSAGEGenerator(kg["n_ent"], kg["n_rel"], dim=args.dim, z_dim=args.z_dim).to(device)
    D = KGSAGEDiscriminator(dim=args.dim).to(device)

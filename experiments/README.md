...
1. Train the generator (Phases 1–2): `DATASET=fb15k237 SEED=0 sbatch experiments/kgsage/slurm/train.slurm` (per-epoch snapshots).
2. Select the snapshot: `python experiments/kgsage/cli/knockout_eval.py --ckpt <each .epNN.pt> --data ...` (lowest mean knockout J@10 wins — the most anchor-specific generator); promote to `generator_<dataset>.pt`.

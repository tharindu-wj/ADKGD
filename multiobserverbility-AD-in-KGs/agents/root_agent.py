...
The root is the circumstance that makes the two auditors different people;
it is not the author of their views -- each auditor writes its own norms
...
ROOT_INSTRUCTION = f"""\
Two auditors are about to judge a dataset for anomalies. You decide what KIND
of judge each one is.

All anyone knows about the dataset, including you, is this:
  "{DATASET.CARD}"

Give each auditor a PERSONA: the values and stance it brings to anything it
...
-- different enough that the two auditors could disagree about the very same
fact. Perspectives that differ only in which topics they mention are the
same judge twice.

Call assign_perspective once for sub_agent_1 and once for sub_agent_2.
...
root = Agent(
    model=MODEL,
    name="root",
    description=("Assigns both auditors their personas from the dataset "

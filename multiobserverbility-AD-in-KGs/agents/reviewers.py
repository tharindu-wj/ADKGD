"""The second-opinion phase: each auditor returns to judge a further batch.

Same auditor, same persona, same recorded norms -- a different ADK agent
name only because one tree cannot hold two agents called sub_agent_1. The
...
This phase exists because two auditors reading different shortlists rarely
...
from tools.auditors import REVIEWER_SUFFIX, SUB_AGENT_NAMES, state_key
...
You are an auditor of a knowledge graph called {DATASET.NAME}, mid-audit.
...
def make_reviewer(principal: str) -> Agent:
    """The returning auditor for one principal."""
    persona_key = state_key("persona", principal)
    return Agent(
        model=MODEL,
        name=principal + REVIEWER_SUFFIX,
        description="The same auditor, judging a further batch by its norms.",
...
reviewers = [make_reviewer(name) for name in SUB_AGENT_NAMES]
